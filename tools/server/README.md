...
| `-mcs, --max-concurrent-streams N` | caps the worst-case number of simultaneously-active streams used to reserve the decode workspace (graph activations / QKV buffers) and the host logits/sampling output buffers, independent of `-np`/`--parallel` (default: 0, 0 = -np). With `--kv-unified` the KV body is a single shared stream, so this reclaims the `-np`-scaled decode-workspace and output-buffer reservation without reducing KV slot capacity or RAM prompt-cache slots. The output buffers are derived as the stream count times the per-sequence speculative expansion (MTP/DFlash emit `1 + n_draft` rows per stream), floored at one output per sequence. Must be `<= -np`; the server never batches more active streams than this into a single decode.<br/>(env: LLAMA_ARG_MAX_CONCURRENT_STREAMS) |
