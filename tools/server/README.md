...
| `-nom, --n-outputs-max N` | caps the worst-case number of concurrently-output streams used to reserve the graph and logits buffers, independent of `-np`/`--parallel` (default: 0, 0 = -np). With `--kv-unified` the KV body is a single shared stream, so this reclaims the `-np`-scaled graph/logits reservation without reducing KV slot capacity or RAM prompt-cache slots. The reserved total is the capped stream count times the per-sequence speculative expansion (MTP/DFlash emit `1 + n_draft` rows per stream), floored at one output per sequence. Only reduce it below `-np` when you never batch more concurrent output streams than the cap allows.<br/>(env: LLAMA_ARG_N_OUTPUTS_MAX) |
