...
static constexpr __host__ __device__ int calc_rows_per_block(int ncols_dst, int table_id, bool small_k = false, int nwarps = 1) {
    if (table_id == MMVQ_PARAMETERS_GENERIC || table_id == MMVQ_PARAMETERS_VOLTA || table_id == MMVQ_PARAMETERS_GCN || table_id == MMVQ_PARAMETERS_TURING || table_id == MMVQ_PARAMETERS_GB10) {
        switch (ncols_dst) {
            case 1:
...
static __global__ void mul_mat_vec_q(
    ...
    // The launch may use fewer warps/rows than the table's maximum (host-side
    // env override, e.g. GGML_CUDA_MMVQ_GENERIC=1 on Volta), so the local
    // arrays below are sized for the compile-time maximum while all loop and
    // addressing math uses the runtime values passed by the host.
    constexpr int nwarps_max              = calc_nwarps(type, ncols_dst, table_id, small_k, halve_iters);
    constexpr int rows_per_cuda_block_max = calc_rows_per_block(ncols_dst, table_id, small_k, nwarps_max);
...
#pragma unroll
    ...
    float tmp[ncols_dst][rows_per_cuda_block_max] = {{0.0f}};
    float tmp_gate[ncols_dst][rows_per_cuda_block_max] = {{0.0f}};
...
#pragma unroll
    ...
    __shared__ float tmp_shared[nwarps_max-1 > 0 ? nwarps_max-1 : 1][ncols_dst][rows_per_cuda_block_max][warp_size];
    [[maybe_unused]] __shared__ float tmp_shared_gate[(has_fusion && (nwarps_max-1 > 0)) ? nwarps_max-1 : 1][ncols_dst][rows_per_cuda_block_max][warp_size];
...
template<ggml_type type>
static mmvq_launch_params calc_launch_params(
    ...
    return {block_nums, block_dims, nwarps, rpb};
}
...
static void mul_mat_vec_q_switch_fusion(
        ...
        const uint32_t ids_stride, const int nwarps, const int rows_per_cuda_block, cudaStream_t stream) {
    ...
    switch (ncols_dst) {
        case 1: {
            ...
            const auto launch = [&](auto small_k_tag, auto halve_iters_tag) {
                ...
                // Check the tables that use the halve_iters mechanism.
                constexpr bool c_promoted =
                    (calc_nwarps(type, c_ncols_dst, MMVQ_PARAMETERS_VOLTA, false, true) !=
                     calc_nwarps(type, c_ncols_dst, MMVQ_PARAMETERS_VOLTA, false, false)) ||
                ...
                mul_mat_vec_q_switch_fusion<type, c_ncols_dst, c_small_k, c_halve_iters>(
                    ...
                    stride_sample_x, stride_sample_y, stride_sample_dst, dims.block_nums, dims.block_dims, 0, ids_stride,
                    dims.nwarps, dims.rows_per_cuda_block, stream);
            };
        ...
        case 2: {
            ...
            mmvq_launch_params dims = calc_launch_params<type>(c_ncols_dst, nrows_x, nchannels_dst, nsamples_dst, warp_size, table_id);
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(vx, vy, ids, fusion, dst, ncols_x, nchannels_y_fd, stride_row_x, stride_col_y, stride_col_dst,
                 channel_ratio_fd, stride_channel_x, stride_channel_y, stride_channel_dst,
                 sample_ratio_fd, stride_sample_x, stride_sample_y, stride_sample_dst,
                 dims.block_nums, dims.block_dims, 0, ids_stride, dims.nwarps, dims.rows_per_cuda_block, stream);
        } break;
        case 3: {
