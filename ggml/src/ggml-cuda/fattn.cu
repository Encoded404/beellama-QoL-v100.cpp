...
bool ggml_cuda_flash_attn_ext_tail_supported(
    ...
    const bool tail_supported = (tail_k == GGML_TYPE_F16 || tail_k == GGML_TYPE_BF16) &&
        (tail_v == GGML_TYPE_F16 || tail_v == GGML_TYPE_BF16);
    const bool tail_quantized = ggml_is_quantized(tail_k) && ggml_is_quantized(tail_v);
    return body_supported &&
        (tail_supported || tail_quantized) &&
        (tail_supported || ggml_cuda_fattn_pair_compiled(tail_k, tail_v)) &&
...
static bool ggml_cuda_flash_attn_ext_tail_pass_supported(int device, const ggml_tensor * dst) {
    ...
    const bool tail_quantized = (kt != nullptr && ggml_is_quantized(kt->type)) ||
            (vt != nullptr && ggml_is_quantized(vt->type));
    // Must match the runtime wrapper's compute-stride choice in fattn-tail.cuh:
    // quantized tails always use the padded vec path.
    const int64_t compute_stride = tail_quantized ?
            int64_t(GGML_PAD(tail_stride, FATTN_KQ_STRIDE)) :
            ggml_cuda_tail_compute_stride(tail_stride);
