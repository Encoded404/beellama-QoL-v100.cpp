...
// Borrowed-context behavior, used by an auxiliary (draft/MTP) context that
// reads a target context's KVarN cache and appends its speculative rows at
// the target's verified frontier while the target is the only writer that may
// seal record groups.
//
//   seal_clamp_groups:  last FULLY VERIFIED record group (target frontier /
//                       128). All record-domain reads are clamped to this
//                       group index. 0 = no clamp (target context behavior).
//   eager_records:      when false, KVARN_STORE generated by this context
//                       completes no record group (rows stay in the F16
//                       staging region until the target re-writes them).
//   advance_fill:       when false, KVARN_STORE does not move the stream's
//                       committed (sealable) fill position. The stage rows
//                       are written, but sealing is driven exclusively by the
//                       target's own eager stores.
struct llama_kvarn_borrow_config {
    uint32_t seal_clamp_groups = 0;
    bool eager_records = true;
    bool advance_fill = true;
};
...
public:
    llama_kv_cache_kvarn_context(
            ...
            const llama_kvarn_borrow_config * borrow_cfg = nullptr);

    // Configure this context instance as a borrower of the target cache one
    // ubatch cycle at a time (serialized on the scheduler thread). The target
    // context's own instances never call this and keep stock behavior.
    void set_borrow(const llama_kvarn_borrow_config & cfg) { borrow_cfg = cfg; }
    const llama_kvarn_borrow_config & get_borrow() const { return borrow_cfg; }
...
private:
    ...
    llama_kvarn_borrow_config borrow_cfg;
...
public:
    ...
    ggml_tensor * store(
            ...
            bool value,
            const llama_kvarn_borrow_config * borrow_cfg = nullptr) const;
    ggml_tensor * view(
            ggml_context * ctx,
            ggml_tensor * stored,
            int32_t il,
    ...
    // Borrowed-context fingerprint: stable parameters a restore descriptor
    // must match before an auxiliary context may resume borrowing.
    uint32_t get_key_bits()   const { return params.key_bits; }
    uint32_t get_value_bits() const { return params.value_bits; }
    uint32_t get_record_groups_per_stream() const { return n_groups_per_stream; }
